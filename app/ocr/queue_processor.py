...
class CaptureQueueProcessor:
    ...
    def _process_market_inventory_task(self, task: CaptureTask) -> None:
        ...
        try:
            self.service.add_market_inventory_capture(market_silver, inventory_silver)
            logger.info(
                f"Market/Inventory capture saved: market={market_silver} inventory={inventory_silver}"
            )
        except Exception as error:
            logger.error(f'Failed to save market/inventory capture: {error}')
    ...
    def _process_storage_snapshot_task(self, task: CaptureTask) -> None:
        ...
        try:
            _, record = self.service.add_storage_capture(storage_name, silver_value)
            ...
            logger.error(f'Failed to save storage capture: {error}')
